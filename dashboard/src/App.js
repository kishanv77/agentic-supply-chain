...
import { ShieldCheck, Truck, Leaf, AlertTriangle, Activity, Thermometer, Droplets, Clock } from 'lucide-react';
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          ...
          <div className="glass-panel animate-slide-up" style={{ padding: '2rem', animationDelay: '0.6s' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {shipments.map(ship => (
                <div key={ship.id} className="data-row" style={{ alignItems: 'center' }}>
                  <div style={{ flex: '1' }}>
                    <div style={{ fontWeight: '600', fontSize: '1.1rem', marginBottom: '4px' }}>{ship.id}</div>
                    <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>{ship.cargo} • {ship.origin} → {ship.destination}</div>
                  </div>
